...
sub main {
	...
	foreach my $patient (sort keys %{$smp_data}) {
		...
		my @all_samples = @tumour_ids;
		push @all_samples, @normal_ids;
		...
		foreach my $sample (@all_samples) {
			...
			# for this step)
			my $input_bam = basename($smp_data->{$patient}->{$type}->{$sample});

			# downsample if requested
			$run_id = '';
			unless ($should_downsample) {

				my $bam = $smp_data->{$patient}->{$type}->{$sample};
				my $index = $bam;
				$index =~ s/bam$/bai/;
			...
			my $wig = join('/', $tmp_directory, $sample . '.wig');
			...
			if ('normal' eq $type) { push @normal_wigs, $wig; }

			# check if this should be run
			if ('Y' eq missing_file($wig)) {
				...
				push @{$patient_jobs{$patient}}, $run_id;
				push @all_jobs, $run_id;
				if ('normal' eq $type) { push @pon_jobs, $run_id; }
				} else {
	...
	foreach my $patient (sort keys %{$smp_data}) {
		...
		my $dependencies = join(':', @{$patient_jobs{$patient}}, $pon_job_id);
		...
		foreach my $sample (@all_samples) {
			...
			# find relevant wig files
			my $sample_wig = join('/', $tmp_directory, $sample . '.wig');
			...
			my $ichor_command = get_ichor_cna_command(
				tumour_id	=> $sample,
				tumour_wig	=> $sample_wig,
				normal_wig	=> ('tumour' eq $type) ? $normal_wig : undef,
				chroms		=> join("','", @chroms),
