...
sub main {
	...
	my $log_directory = join('/', $output_directory, 'logs', $tool_data->{project_name});
	...
	if ($run_wgs) {
		$tool_dirs{swgs_bams} = join('/', $wgs_directory, 'BWA');
		$tool_dirs{swgs_qc} = join('/', $wgs_directory, 'BAMQC');
		$tool_dirs{swgs_ichor} = join('/', $wgs_directory, 'IchorCNA');
		}

	if ($run_dna) {
		$tool_dirs{dnaseq_bams} = join('/', $dna_directory, 'GATK');
		$tool_dirs{dnaseq_qc} = join('/', $dna_directory, 'BAMQC');
	...
	if ($run_em) {
		$tool_dirs{emseq_bams} = join('/', $em_directory, 'BWA');
	...
	my ($wgs_data_config, $dna_data_config, $em_data_config); 
	...
	if ($run_wgs) {
		...
		# find BAM yaml
		opendir(FILES, $tool_dirs{'swgs_bams'});
		@found_files = grep { /yaml/ } readdir(FILES);
		@found_files = sort @found_files;
		closedir(FILES);

		$wgs_data_config = join('/', $tool_dirs{'swgs_bams'}, $found_files[-1]);
		print $log "\n  Sample config used for sWGS: $wgs_data_config\n";
		...
		my @cn_files = grep { /ichorCNA_segments.tsv/ } @found_files;
		...
		$link_file = join('/', $ichor_dir, $cn_files[-1]);
	...
	if ($run_dna) {
		...
		# find BAM yaml
		opendir(FILES, $tool_dirs{'dnaseq_bams'});
		@found_files = grep { /yaml/ } readdir(FILES);
		@found_files = sort @found_files;
		closedir(FILES);

		$dna_data_config = join('/', $tool_dirs{'dnaseq_bams'}, $found_files[-1]);
		print $log "\n  Sample config used for DNA-Seq: $dna_data_config\n";
		...
		$link_file = join('/', $qc2_dir,
			$tool_data->{project_name} . '_' . $contam_files[-1]
			);
		...
		$qc_command .= " " . join(' ',
			...
			'-s', $dna_data_config,
			'-c', join('/', $qc_dir, $cov_files[-1]),
			'-g', join('/', $qc2_dir, $tool_data->{project_name} . '_' . $contam_files[-1]),
			'-t dnaseq'
			);
		...
		$ensemble_maf = join('/',
			...
			'DNASeq',
			'ENSEMBLE',
			$tool_data->{project_name} . '_ensemble_mutation_data.tsv'
			);

		# add MAF to summary command
		$summarize_mutations_command .= " -m $ensemble_maf";
		$summarize_panel_command .= " -e $ensemble_maf";
	...
	if ($run_em) {
		...
		# find BAM yaml
		opendir(FILES, $tool_dirs{'emseq_bams'});
		@found_files = grep { /yaml/ } readdir(FILES);
		@found_files = sort @found_files;
		closedir(FILES);

		$em_data_config = join('/', $tool_dirs{'emseq_bams'}, $found_files[-1]);
		print $log "\n  Sample config used for EM-Seq: $em_data_config\n";
	...
	my $smp_data = LoadFile($dna_data_config);
	...
	foreach my $patient (sort keys %{$smp_data}) {
		...
		my $report_file =  "multiMMR_report__" . $patient . ".tex";
...
# declare variables
my ($wgs_tool_config, $dna_tool_config, $em_tool_config);
...
GetOptions(
	'h|help'	=> \$help,
	'w|wgs_tool=s'	=> \$wgs_tool_config,
	'd|dna_tool=s'	=> \$dna_tool_config,
	'e|em_tool=s'	=> \$em_tool_config,
...
if ($help) {
	my $help_msg = join("\n",
		...
		"\t--wgs_tool|-w\t<string> tool config for sWGS (yaml format)",
		"\t--dna_tool|-d\t<string> tool config for DNA-Seq (yaml format)",
		"\t--em_tool|-e\t<string> tool config for EM-Seq (yaml format)",
...
if (!defined($wgs_tool_config)) {
	print("Config file for sWGS is missing; will skip this portion.\n");
	}

if (!defined($dna_tool_config)) {
	print("Config file for DNA-Seq is missing; will skip this portion.\n");
	}

if (!defined($em_tool_config)) {
	print("Config file for EM-Seq is missing; will skip this portion.\n");
	}
