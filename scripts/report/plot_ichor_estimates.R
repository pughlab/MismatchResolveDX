...
tf.data <- tf.data[order(tf.data$Type, -tf.data$Tumour.Fraction),];
tf.data$Sample <- factor(tf.data$Sample, levels = tf.data$Sample);

# determine some parameters
axis.cex <- if (nrow(tf.data) <= 30) { 1
	} else if (nrow(tf.data) <= 70) { 0.8
...
tf.plot <- create.barplot(
	...
	ylab.axis.padding = 2,
	right.padding = 0,
	xaxis.lab = simplify.ids(as.character(tf.data$Sample)),
	xaxis.rot = 90,
	xaxis.tck = c(0.5,0),
	yaxis.tck = c(0.5,0),
	xaxis.cex = axis.cex,
...
tf.data$Sample <- as.character(tf.data$Sample);
...
for (file in all.files) {
	load(file);
	combined.metrics <- unique(rbind(combined.metrics, tf.data));
	}
...
for (patient in unique(orig.sample.info$Patient)) {
	...
	to.write <- smp.data[,c('Sample','Sex','Tumour.Fraction')];
	colnames(to.write) <- c('Sample','Inferred Sex','Tumour Fraction');

	to.write[,3] <- round(to.write[,3],3);

	tf.legend <- list(
		text = list(lab = paste0(to.write$Sample, ' = ', to.write[,3]), cex = 0.8)
		);
	...
	smp.plot <- create.polygonplot(
		...
		ylab.axis.padding = 2,
		legend = list(
			inside = list(fun = my.legend, x = 0, y = 1.1),
			inside = list(fun = draw.key, args = list(key = tf.legend), x = 0.8, y = 1)
			),
		abline.v = smp.data$Tumour.Fraction,
		abline.lty = c(2,1)[match(smp.data$Type, c('normal','tumour'))],
