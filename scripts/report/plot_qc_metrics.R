...
if (!is.null(concordance.data)) {
	...
	total.gtchecked <- max(concordance.data$Total.Sites, na.rm = TRUE);
...
axis.cex <- if (length(smp.names) <= 30) { 1
	} else if (length(smp.names) <= 70) { 0.8
...
percent.key <- list(
	...
	args = list(
		key = list(
			...
			text = list(
				lab = paste0('Percent bases above ', target_coverage, 'x'),
				cex = 1
				)
			)
		),
...
total.coverage.plot <- create.scatterplot(
	...
	legend = list(top = cov.key, inside = percent.key)
	);
...
if ('wgs' != arguments$seq_type) {
	...
	stacked.data$Group <- factor(
		stacked.data$Group,
		levels = c('percent_on_bait','percent_near_bait','percent_off_bait')
		);

	bar.scheme <- c('violetred3','turquoise3','seagreen3');
	names(bar.scheme) <- c('percent_off_bait','percent_near_bait','percent_on_bait');

	stack.legend <- list(fun = draw.key, args = list(key = list(
		space = 'top',
		rect = list(col = bar.scheme, size = 1),
		text = list(lab = c('off target', 'near target', 'on target'), cex = 1),
		columns = 3,
		between = 0.7
		)), x = 0.02, y = 1);
	...
	efficiency.plot <- create.barplot(
		...
		stack = TRUE,
		col = bar.scheme[levels(stacked.data$Group)],
		xlab.label = NULL,
...
for (patient in unique(orig.sample.info$Patient)) {
	...
	if (!is.null(concordance.data)) {
		...
		if (length(these.smps) > 1) {
			plot.data <- orig.cor.data[grepl(patient, rownames(orig.cor.data)),];
			} else {
			plot.data <- rbind(
				orig.cor.data[grepl(patient, rownames(orig.cor.data)),],
				orig.cor.data[grepl(patient, rownames(orig.cor.data)),]
				);
			}
		...
		create.heatmap(
			...
			xlab.label = '',
			x.alternating = 2,
			xaxis.lab.top = colnames(plot.data),
			yaxis.lab = if (length(these.smps) == 1) { c(patient,'') } else { NA },
			xaxis.cex = axis.cex,
			yaxis.cex = axis.cex,
			xaxis.tck = if (ncol(plot.data) <= 50) { 0.2 } else { 0 },
			yaxis.tck = if (length(these.smps) == 1) { 0 
				} else if (nrow(plot.data) <= 50) { 0.2 } else { 0 },
			xaxis.fontface = 'plain',
			yaxis.fontface = 'plain',
			colourkey.cex = 1,
			colour.scheme = c('white','black'),
			...
			colourkey.labels.at = c(0.8,0.9,0.95,1),
			colourkey.labels = c('< 0.8','0.90', '0.95', '1'),
			grid.row = if (length(these.smps) > 1) { TRUE } else { FALSE },
			force.grid.row = if (length(these.smps) > 1) { TRUE } else { FALSE },
			row.colour = 'grey80',
			col.colour = 'grey80',
			grid.col = TRUE,
			force.grid.col = TRUE,
			...
			width = 11,
			resolution = 200,
			file = generate.filename(patient, 'picard_GTcheck','png')
			);

		caption2 <- paste0('GT concordance across ', total.gtchecked, ' variants.');

		write("\\begin{figure}[h!]", file = tex.file, append = TRUE);
		write("\\begin{center}", file = tex.file, append = TRUE);
		write(paste0(
			"\\includegraphics[width=0.9\\textwidth]{",
			generate.filename(patient, 'picard_GTcheck','png'), '}'
